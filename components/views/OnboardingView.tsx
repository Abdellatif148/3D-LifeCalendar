...
interface OnboardingViewProps {
  onComplete: (_data: LifeData) => void;
}
...
const OnboardingView: React.FC<OnboardingViewProps> = ({ onComplete }) => {
  ...
  const updateUnallocated = (activities: ActivityData[]) => {
    ...
    const finalActivities = activities.map((act) =>
      act.name === 'Unallocated'
        ? { ...act, minutesPerDay: unallocatedMinutes }
        : act
    );
  ...
  const unallocatedHours =
    (localData.activities.find((a) => a.name === 'Unallocated')
      ?.minutesPerDay ?? 0) / 60;
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-900">
      <Card className="w-full max-w-2xl">
        ...
        <p className="text-center text-gray-400 mb-6">
          Let's set up your baseline. How do you spend a typical day?
        </p>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalData({
                    ...localData,
                    currentAge: parseInt(e.target.value, 10) || 0,
                  })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setLocalData({
                    ...localData,
                    targetAge: parseInt(e.target.value, 10) || 0,
                  })
                }
                className="w-full bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
                required
              ...
              .map((activity) => (
                <div key={activity.name}>
                  <label
                    ...
                    style={{ color: CATEGORY_MAP[activity.name].color }}
                  >
          ...
          <div className="text-center bg-gray-700 p-3 rounded-md">
            <p className="font-semibold">
              ...
              <span
                ...
                style={{ color: CATEGORY_MAP['Unallocated'].color }}
              >
                {unallocatedHours.toFixed(1)} hours/day
              </span>
            </p>
          </div>
