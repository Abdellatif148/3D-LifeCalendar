...
import type { Delta, ActivityData, CategoryName } from '../../types';
...
const DashboardView: React.FC<DashboardViewProps> = ({ onReset }) => {
  ...
  const handleSliderChange = (name: CategoryName, deltaMinutes: number) => {
    ...
    setDeltas((prevDeltas) => {
      ...
      if (deltaMinutes !== 0) {
        return [...otherDeltas, { name, deltaMinutes }];
      }
      return otherDeltas;
    });
  };
  ...
  const dominantFutureActivity = useMemo(() => {
    const futureActivities = lifeData.activities.filter(
      (a) => a.name !== 'Sleep' && a.name !== 'Unallocated'
    );
    if (futureActivities.length === 0) return 'Unallocated';
    return futureActivities.reduce((max, act) =>
      act.minutesPerDay > max.minutesPerDay ? act : max
    ).name;
  }, [lifeData.activities]);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 gap-0 relative">
        ...
        <div className="lg:col-span-3 bg-gray-900/50 p-4 overflow-y-auto z-10">
          <MetricsPanel
            deltas={deltas}
            yearsLeft={lifeData.targetAge - lifeData.currentAge}
          />
        </div>
      </div>
    </div>
