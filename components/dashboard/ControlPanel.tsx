...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const goalableActivities = activities.filter(
    (a) => a.name !== 'Unallocated' && a.name !== 'Sleep'
  );
  const [goalActivity, setGoalActivity] = useState<CategoryName>(
    goalableActivities[0]?.name || 'Exercise'
  );
    ...
    <div className="space-y-6">
            ...
            >
              <span className="flex items-center">
                <span
                  ...
                  style={{ backgroundColor: CATEGORY_MAP[act.name].color }}
                ></span>
                {act.name}
              </span>
              <div className="flex items-center gap-4">
                <span className="font-semibold text-gray-300 w-16 text-right">
                  {((act.minutesPerDay / MINUTES_IN_DAY) * 100).toFixed(1)}%
                </span>
      ...
      <Card>
        ...
        <p className="text-sm text-gray-400 mb-4">
          Define a specific goal to see its impact. This will adjust the
          simulator below.
        </p>
        <form onSubmit={handleAddGoal} className="space-y-3">
          <div className="flex gap-2">
            ...
            <input
              ...
              value={goalMinutes}
              onChange={(e) =>
                setGoalMinutes(parseInt(e.target.value, 10) || 0)
              }
          ...
          <div className="mt-4 space-y-2">
            ...
            <ul className="space-y-1">
              {deltas.map((delta) => (
                <li
                  key={delta.name}
                  className="flex justify-between items-center text-sm bg-gray-700/50 p-2 rounded-md"
                >
                  <span className="flex items-center">
                    <span
                      className="w-2 h-2 rounded-full mr-2"
                      style={{
                        backgroundColor: CATEGORY_MAP[delta.name].color,
                      }}
                    ></span>
                    {delta.name}:{' '}
                    <span className="font-bold ml-1">
                      {delta.deltaMinutes > 0 ? '+' : ''}
                      {delta.deltaMinutes} min
                    </span>
                  </span>
                  <button
      ...
      <Card>
        ...
        <p className="text-sm text-gray-400 mb-4">
          Adjust your daily routine and see the lifetime impact instantly. Each
          slider adds or subtracts up to 2 hours per day.
        </p>
        <div className="space-y-4">
          {activities
            .filter((a) => a.name !== 'Unallocated' && a.name !== 'Sleep')
            .map((act) => {
              const baseValue =
                baseActivities.find((a) => a.name === act.name)
                  ?.minutesPerDay ?? 0;
              ...
              return (
                <Slider
                  ...
                  value={delta}
                  onChange={(e) =>
                    onSliderChange(act.name, parseInt(e.target.value))
                  }
                />
              );
            })}
