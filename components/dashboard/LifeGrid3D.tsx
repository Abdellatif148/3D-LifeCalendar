...
const WeekBlock: React.FC<WeekBlockProps> = React.memo(({ position, color, isPast, onPointerOver, onPointerOut }) => {
...
const LifeGrid: React.FC<LifeGridProps> = ({ currentAge, targetAge, dominantColorActivity }) => {
    ...
    const [hovered, setHovered] = useState<{ year: number; week: number; position: [number, number, number]; color: string; isPast: boolean } | null>(null);
        ...
        <group ref={groupRef} rotation={[0.2, 0, 0]}>
            {weeks.map(w => 
                <WeekBlock 
                    ...
                    onPointerOver={(e) => {
                        e.stopPropagation();
                        setHovered({ year: w.year, week: w.week + 1, position: w.position, color: w.color, isPast: w.isPast });
                    }}
                    onPointerOut={() => {
                        setHovered(null);
                    }}
